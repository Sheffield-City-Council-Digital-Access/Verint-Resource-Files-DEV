...
function handleOnReadyKnowledge(knowledge) {
  let redirectToForm = '';
  ...
  $('.back-btn').on('click', function () {
    hideShowElement(this.name, 'hide');
    const backPages = {
      'but_back_subject_menu': 'page_subject_menu',
      'but_back_topic_menu': 'page_topic_menu',
      'but_back_search_results': 'page_search_results',
      'but_back_content': 'page_content',
      'but_back_latest_news': 'page_latest_news',
      'but_back_service_a_z': 'page_service_a_z'
    };
    if (backPages[this.name]) {
      hideShowElement(backPages[this.name], 'show');
    }
  });
  ...
  function redirectToContentPage(item) {
    if (item.content) {
      redirectToForm = item.process ? item.process.formName || '' : '';
      ...
      const button = document.getElementById('dform_widget_button_but_launch_process');
      if (item.process && item.process.buttonLabel) {
        button.textContent = item.process.buttonLabel;
        hideShowElement('but_launch_process', 'show');
      } else {
        hideShowElement('but_launch_process', 'hide');
      }
...
function handleServicesAtoZ(knowledge) {
  const resetFilter = document.querySelector('.reset-filter');
  const aToZFilter = document.querySelector('.a-z-filter');
  const categoriesList = document.querySelector('.categories ul');
  const optionsContainer = document.querySelector('.options');
  ...
  function createAtoZFilter() {
    for (let i = 65; i <= 90; i++) {
      ...
      const button = document.createElement('button');
      button.textContent = letter;
      button.disabled = !knowledge.some(item =>
  ...
  function createOptions(itemsToDisplay = knowledge) {
    optionsContainer.innerHTML = '';
    itemsToDisplay
      .filter(item => item.content) // Only items with content
      .sort((a, b) => a.name.localeCompare(b.name))
      .forEach(item => {
        const optionDiv = document.createElement('div');
        optionDiv.classList.add('option');
        optionDiv.innerHTML = `<h4>${item.name}</h4><p>${item.description}</p>`;
        optionDiv.addEventListener('click', () => {
  ...
  function filterOptions(letter) {
    const filteredItems = knowledge.filter(item => item.name.toUpperCase().startsWith(letter));
    createOptions(filteredItems);
  }

  function filterByCategory(category) {
    const filteredItems = knowledge.flatMap(item =>
      item.subjects ? item.subjects.filter(subject => subject.name === category) : []
    );
    createOptions(filteredItems);
  }
